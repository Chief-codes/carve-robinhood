...
import {client,DEPLOYMENT,CURVE_DEPLOYMENT,type Deployment,LEGACY_DEPLOYMENT,V3_DEPLOYMENT,TOKEN_ABI,MARKET_ABI,ENGINE_ABI,ROUTER_ABI} from '../lib/chain';
...
export function InscriptionProof({token,registry,roots}:{token:Address;registry:Address;roots:(Hex|null)[]}){
 ...
 {files.filter(f=>f.mime==='text/html').map(f=>{const capsule=readCapsuleHTML(f.bytes);return capsule?<CapsuleDetails key={f.root} capsule={capsule}/>:null;})}
 </section>;
}

export function TokenTrading(props:{token:Address;symbol:string}){
 const [kind,setKind]=useState<'loading'|'curve'|'previous'|'v3'|'legacy'|'other'|'error'>('loading');
 useEffect(()=>{let active=true;setKind('loading');
  (async()=>{
   let next:'curve'|'previous'|'v3'|'legacy'|'other'='other';
   if([4,5].includes(DEPLOYMENT.curveVersion||0)&&await findMarket(client,props.token,DEPLOYMENT))next='curve';
   else if(await findMarket(client,props.token,CURVE_DEPLOYMENT))next='previous';
 ...
 if(kind==='previous')return <LegacyTokenTrading key={props.token+':previous'} {...props} deployment={CURVE_DEPLOYMENT}/>;
...
function LegacyTokenTrading({token,symbol,deployment=DEPLOYMENT}:{token:Address;symbol:string;deployment?:Deployment}){
 ...
 const {w,getSession}=useTransactionSession(p=>{if(alive.current)setProgress(p);},deployment);
 ...
 const refresh=async()=>{
  ...
  const m=await findMarket(client,token,deployment);if(!valid())return;setMarket(m);if(!m)return;
