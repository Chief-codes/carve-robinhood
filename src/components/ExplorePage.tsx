...
export function ExplorePage(){
 ...
 {result&&<div className="onchain-sections">{result.registry&&result.inscribed&&<InscriptionProof key={result.address+result.registry} token={result.address as Address} registry={result.registry} roots={result.roots}/>}<TokenTrading key={result.address+':'+w.address+':'+w.selected?.info.uuid} token={result.address as Address} symbol={result.symbol}/></div>}
 {(result||address)&&<LaunchFeed/>}
 </>;
}
