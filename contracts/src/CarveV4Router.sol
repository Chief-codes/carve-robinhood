...
///      Native sale proceeds and unspent buy ETH use caller-owned pull credits.
contract CarveV4Router is CarveEscrow {
    ...
    constructor(CarveV4Engine engine_) {
    ...
    function _swap(Order memory order) private returns (uint256 input, uint256 output) {
        ...
        if (order.buy && maxInput > input) _credit(order.account, maxInput - input);
        if (!order.buy) _credit(order.account, output);
