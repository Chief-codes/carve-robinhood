...
import {CarveCurveMarket} from "./CarveCurveMarket.sol";
...
contract CarveCurveFactory is CarveEscrow {
    ...
    {
        return _launch(name, symbol, imageRoot, audioRoot, websiteRoot, creatorFeeBps, minTokensOut, deadline);
    }
    ...
    function launchInline(string calldata name, string calldata symbol, CarveInlineContent.Asset[3] calldata assets,
        uint16 creatorFeeBps, uint256 minTokensOut, uint256 deadline)
        external payable nonReentrant returns (address token, address market)
    {
        bytes32[3] memory roots = CarveInlineContent.prepare(registry, assets);
        return _launch(name, symbol, roots[0], roots[1], roots[2], creatorFeeBps, minTokensOut, deadline);
    }
        ...
        private returns (address token, address market)
    {
        ...
        CarveCurveMarket deployed = new CarveCurveMarket{value: msg.value - creationFee}(
            msg.sender, address(registry), name, symbol, imageRoot, audioRoot, websiteRoot, creatorFeeBps,
            CarveCurveMarket.Config(supply, virtualETH, capETH, creatorFeeLimitBps, platformRecipient, migrationAdapter, locker),
