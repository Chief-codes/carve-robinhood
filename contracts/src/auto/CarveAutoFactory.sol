...
import {CarveAutoMarket} from "./CarveAutoMarket.sol";
import {CarveInlineContent} from "../v3/CarveInlineContent.sol";
...
contract CarveAutoFactory is CarveEscrow {
    uint256 public constant version = 5;
    uint256 public constant maxInlineBytes = 24_576;
    ...
    function launch(string calldata name, string calldata symbol, bytes32 imageRoot, bytes32 audioRoot,
        bytes32 websiteRoot, uint16 creatorFeeBps, bool autoBuyback, uint256 minTokensOut, uint256 deadline)
        external payable nonReentrant returns (address token, address market)
    {
        return _launch(name, symbol, imageRoot, audioRoot, websiteRoot, creatorFeeBps, autoBuyback, minTokensOut, deadline);
    }

    /// @notice Any nonempty image/audio/HTML combination; small files and initial buy in one request.
    function launchInline(string calldata name, string calldata symbol, CarveInlineContent.Asset[3] calldata assets,
        uint16 creatorFeeBps, bool autoBuyback, uint256 minTokensOut, uint256 deadline)
        external payable nonReentrant returns (address token, address market)
    {
        bytes32[3] memory roots = CarveInlineContent.prepare(registry, assets);
        return _launch(name, symbol, roots[0], roots[1], roots[2], creatorFeeBps, autoBuyback, minTokensOut, deadline);
    }

    function _launch(string memory name, string memory symbol, bytes32 imageRoot, bytes32 audioRoot,
        bytes32 websiteRoot, uint16 creatorFeeBps, bool autoBuyback, uint256 minTokensOut, uint256 deadline)
        private returns (address token, address market)
    {
        ...
        CarveAutoMarket deployed = new CarveAutoMarket{value: msg.value - creationFee}(
            msg.sender, address(registry), name, symbol, imageRoot, audioRoot, websiteRoot, creatorFeeBps,
            CarveAutoMarket.Config(supply, virtualETH, capETH, creatorFeeLimitBps, platformRecipient, migrationAdapter, locker, autoBuyback),
        ...
        // authenticate it, then attempt its permanently locked v4 position. Failure leaves the funded curve retryable.
        if (deployed.canGraduate()) deployed.tryGraduate(deadline);
