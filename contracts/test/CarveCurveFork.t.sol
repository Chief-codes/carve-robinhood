...
import {CarveCurveEngine, ICarvePositionManager} from "../src/curve/CarveCurveEngine.sol";
...
contract CurveRejectReceiver {
    ...
    function sell(CarveCurveMarket market, CarveToken token, uint256 amount) external {
        token.approve(address(market), amount);
        market.sell(amount, 1, block.timestamp);
    }
    function withdraw(CarveCurveMarket market, address payable recipient) external { market.withdrawETH(recipient); }
}
...
contract CarveCurveForkTest {
    ...
    CarveCurveFactory private factory;
    CarveCurveEngine private engine;
    CarveCurveRouter private router;
    CarveCurveMarket private market;
    ...
    function setUp() public {
        ...
        bytes32 initHash = keccak256(abi.encodePacked(type(CarveCurveEngine).creationCode, abi.encode(PM, predicted)));
        bytes32 salt;
        for (uint256 i; ; ++i) {
            salt = bytes32(i);
        ...
        engine = new CarveCurveEngine{salt: salt}(PM, predicted);
        factory = new CarveCurveFactory(registry, PLATFORM, address(engine));
    ...
    function _launch(uint256 buy, uint16 fee) private {
        (address t, address m) = factory.launchInline{value: 0.0005 ether + buy}(
            "Carve curve verification", "CURVE", _assets(), fee, buy == 0 ? 0 : 1, block.timestamp + 300);
    ...
    function testAllSevenMediaCombinations() public {
        for (uint256 mask = 1; mask < 8; ++mask) {
            ...
            (address t,) = factory.launchInline{value: 0.0005 ether}("Media", "MEDIA", a, 0, 0, block.timestamp);
    ...
    function testInvalidEmptyAndOverLimitContentReverts() public {
        ...
        vm.expectRevert();
        factory.launchInline{value: 0.0005 ether}("Empty", "EMPTY", empty, 0, 0, block.timestamp);
        CarveInlineContent.Asset[3] memory a = _assets(); a[0].data = new bytes(24577);
        vm.expectRevert();
        factory.launchInline{value: 0.0005 ether}("Large", "LARGE", a, 0, 0, block.timestamp);
    ...
    function testSlippageAndSelfEntryCannotSpendWalletFunds() public {
        _launch(0.01 ether, 0);
        vm.expectRevert(CarveCurveMarket.OnlySelf.selector); market.completeGraduation(block.timestamp);
        uint256 beforeETH = address(this).balance;
        vm.expectRevert(CarveCurveMarket.Slippage.selector); market.buy{value: 0.01 ether}(type(uint256).max, block.timestamp);
