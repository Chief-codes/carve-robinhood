...
export const POSITION_MANAGER_CODE_HASH = '0xc873e135dc9aaec88489cfbad146b4cb49d6a32e0d80326377784b7ba17670b2';
export const PERMIT2 = '0x000000000022D473030F116dDEE9F6B43aC78BA3';
export const HOOK_MASK = 0x3fffn;
export const HOOK_FLAGS = 0x2044n;
export const ARTIFACT_NAMES = ['CarveContentRegistry', 'CarveCurveDeployment', 'CarveCurveEngine', 'CarveCurveFactory', 'CarveCurveRouter'];
...
export async function loadArtifacts(projectRoot = PROJECT_ROOT) {
  ...
  const coordinator = artifacts.CarveCurveDeployment.creationCode.toLowerCase();
  for (const name of ['CarveCurveEngine', 'CarveCurveFactory', 'CarveCurveRouter']) {
...
export function makeUnsignedPlan(artifacts, {
  ...
  const engineInitCode = encodeDeployData({ abi: artifacts.CarveCurveEngine.abi,
    bytecode: artifacts.CarveCurveEngine.creationCode, args: [POOL_MANAGER, addresses.factory] });
  ...
  const coordinatorData = encodeDeployData({ abi: artifacts.CarveCurveDeployment.abi,
    bytecode: artifacts.CarveCurveDeployment.creationCode, args: [addresses.registry, platformRecipient, mining.salt] });
  ...
  const core = {
    ...
    canonicalPositionManager: { address: POSITION_MANAGER, codeHash: POSITION_MANAGER_CODE_HASH, permit2: PERMIT2 },
...
export async function verifyCanonicalPositionManager(client, blockNumber) {
  const code = await client.getBytecode({ address: POSITION_MANAGER, blockNumber });
  check(code && keccak256(code) === POSITION_MANAGER_CODE_HASH, 'Canonical PositionManager runtime mismatch');
  const abi = viem.parseAbi(['function poolManager() view returns (address)', 'function permit2() view returns (address)']);
  for (const [functionName, expected] of [['poolManager', POOL_MANAGER], ['permit2', PERMIT2]]) {
    const value = await client.readContract({ address: POSITION_MANAGER, abi, functionName, blockNumber });
    check(getAddress(value) === getAddress(expected), 'PositionManager dependency mismatch');
  }
  check((await client.getBytecode({ address: PERMIT2, blockNumber }))?.length > 2, 'Missing canonical Permit2');
  return { address: POSITION_MANAGER, runtimeCodeHash: keccak256(code), permit2: PERMIT2 };
}
...
export async function readPreflight(client, deployer = APPROVED_WALLET) {
  ...
  const positionManager = await verifyCanonicalPositionManager(client, block.number);
  return { positionManager, chainId, pendingNonce: nonce, observedBlockNumber: block.number.toString(), observedBlockHash: block.hash,
