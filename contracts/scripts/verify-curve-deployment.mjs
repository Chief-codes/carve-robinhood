...
import { viem, loadArtifacts, makeUnsignedPlan, publicClient, json, shortError,
  ...
  verifyReusedRegistry, verifyCanonicalPositionManager, POSITION_MANAGER, POSITION_MANAGER_CODE_HASH, PERMIT2 } from './prepare-curve-deployment.mjs';
...
export function assertPlanMatchesArtifacts(plan, artifacts) {
  ...
  for (const field of ['nonce', 'canonicalPositionManager', 'canonicalPoolManagerCodeHash', 'hook', 'economics', 'artifactHashes', 'transactionDataHashes',
...
export async function verifyDeployment(client, plan, artifacts, transactionHashes) {
  ...
  const contracts = {
    registry: 'CarveContentRegistry', coordinator: 'CarveCurveDeployment', engine: 'CarveCurveEngine',
    factory: 'CarveCurveFactory', router: 'CarveCurveRouter',
  };
  ...
  const canonicalPositionManager = await verifyCanonicalPositionManager(client, blockNumber);
  ...
  const bindings = [
    ...
    ['engine', 'POSITION_MANAGER', POSITION_MANAGER], ['engine', 'POSITION_MANAGER_CODE_HASH', POSITION_MANAGER_CODE_HASH],
    ['engine', 'PERMIT2', PERMIT2], ['factory', 'version', 4n], ['factory', 'maxInlineBytes', 24576n],
  ...
  return {
    ...
    addresses: a, runtimeHashes, canonicalPositionManager, canonicalPoolManagerCodeHash: keccak256(poolCode),
